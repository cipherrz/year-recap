...
<section class="max-w-2xl mx-auto py-20 px-4">
  <div class="sys-card border sys-border-danger bg-black/80 font-mono text-sm">
    <div class="flex justify-between items-center p-2 border-b sys-border-danger bg-[rgba(var(--danger),0.1)]">
      <span class="text-[rgb(var(--danger))] font-bold ml-2">DED_MOROZ_DIRECT_UPLINK</span>
      <div class="flex gap-1 mr-2">
        <div class="w-2 h-2 rounded-full bg-[rgb(var(--danger))]"></div>
        <div class="w-2 h-2 rounded-full bg-[rgb(var(--danger))]"></div>
      </div>
    </div>
      ...
      {#each messages as msg}
        <div class="opacity-100">
          ...
          {:else if msg.from === 'santa'}
            <div class="text-[rgb(var(--danger))] font-bold">DED_MOROZ_BOT: <span class="text-gray-300 font-normal">{msg.text}</span></div>
          {:else}
            <div class="text-[rgb(var(--cyan))] text-right">{msg.text} &lt;</div>
          {/if}
        </div>
      {/each}
    </div>
    ...
    <!-- Input Area -->
    <div class="p-4 border-t border-[rgba(255,45,85,0.3)] flex gap-2">
      <span class="text-[rgb(var(--text-dim))]">></span>
      <input 
        bind:value={input}
        on:keydown={(e) => { if (e.key === 'Enter') send(); }}
        class="bg-transparent w-full outline-none text-[rgb(var(--text-main))]" 
        placeholder="Напиши Деду..."
      />
      <button on:click={send} class="text-[rgb(var(--cyan))] hover:text-white uppercase text-xs">[SEND]</button>
    </div>
  </div>
  
  <div class="mt-4 text-center">
    <button on:click={genWish} class="text-xs text-[rgb(var(--text-log))] hover:text-[rgb(var(--tangerine))] underline decoration-dashed pointer-cursor">
